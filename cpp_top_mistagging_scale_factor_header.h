...
private:
    ...
    ScaleFactorHelper(const std::string& year, bool is_data) {
        ...
        if (is_data) {
            ...
            histo = dynamic_cast<TH2F*>(eff_file->Get("efficiency_tagger_pTVSmass_data_1tag")); // for MC
            if (histo) {
    ...
    std::string getFilePath(const std::string& year) {
        if (year == "2018") {
            ...
            return "tagging_efficiency_for_bkg_estimation/all_years_Run2/tagging_efficiency_from_on_Z_2J_1T_pTVSmass_merged_ee_mumu_poiss_unc_leading_subleading_merged_noHadrTop_finerEffBinning.root";
        } else if (year == "2017") {
