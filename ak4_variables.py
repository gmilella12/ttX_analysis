...
class Processor:
    def __init__(self, input_file, output_dir, year, is_data, is_sgn, weighting, sys):
        ...
        self.output_file = creation_output_file(input_file, output_dir, "ak4_variables", year, EVENT_SELECTION, self.sys) 
        ...
        for lepton_selection in self.lepton_selection:
            ...
            for var in VARIABLES_BINNING.keys():
                ...
                for ijet, jet in enumerate(['leading', 'subleading']):
                    histo_title = "{}_ak4_{}_outside_{}_{}_{}_{}".format(self.process_name, jet, BOOSTED_JETS, var, EVENT_SELECTION, lepton_selection)
                    histo_var = 'selectedJets_nominal_outside_{}_{}_{}'.format(BOOSTED_JETS, var, jet)
                    ...
                    output_histo.Write()
                    print('Total events (AK4 outside): {}'.format(output_histo.Integral()))

                    histo_title = "{}_b_loose_{}_outside_{}_{}_{}_{}".format(self.process_name, jet, BOOSTED_JETS, var, EVENT_SELECTION, lepton_selection)
                    histo_var = 'selectedBJets_nominal_loose_outside_{}_{}_{}'.format(BOOSTED_JETS, var, jet)
                    print(histo_var)
                    if not self.is_data:
                        output_histo = root_df_filtered.Histo1D((histo_title, '', bins, bin_edges), histo_var, 'weight')
                    else:
                        output_histo = root_df_filtered.Histo1D((histo_title, '', bins, bin_edges), histo_var)
                    output_histo.Write()
                    print('Total events (b outside): {}'.format(output_histo.Integral()))
    ...
    def _adding_new_columns(self, root_df):
        ...
        for boosted_jets, boosted_jets_label in zip(['ak8', 'hotvr'], ['selectedFatJets_nominal', 'preselectedHOTVRJets']):
            ...
            for var in VARIABLES_BINNING.keys():
                root_df = root_df.Define("selectedJets_nominal_outside_{}_{}".format(boosted_jets, var), 
                                     "selectedJets_nominal_{}[selectedJets_nominal_outside_{}]".format(var, boosted_jets))
                for ijet, jet in enumerate(['leading', 'subleading']): 
                    root_df = root_df.Define(
                        "selectedJets_nominal_outside_{}_{}_{}".format(boosted_jets, var, jet),
                        "selectedJets_nominal_outside_{}_{}.size() > {} ? selectedJets_nominal_outside_{}_{}[{}] : -99".format(
                            boosted_jets, var, ijet, boosted_jets, var, ijet
                        )
                    ) # always checking the size of the array; if empty, segmentation violation error arises
                    # print("selectedJets_nominal_outside_{}_{}_{}".format(boosted_jets, var, jet))
            # ---
            ...
            for WP in B_TAGGING_WP[str(self.year)].keys():
                ...
                for var in VARIABLES_BINNING.keys():
                    root_df = root_df.Define("selectedBJets_nominal_{}_outside_{}_{}".format(WP, boosted_jets, var), 
                                        "selectedJets_nominal_outside_{}_{}[selectedBJets_nominal_{}_outside_{}]".format(boosted_jets, var, WP, boosted_jets))

                    for ijet, jet in enumerate(['leading', 'subleading']): 
                        root_df = root_df.Define(
                            "selectedBJets_nominal_{}_outside_{}_{}_{}".format(WP, boosted_jets, var, jet),
                            "selectedBJets_nominal_{}_outside_{}_{}.size() > {} ? selectedBJets_nominal_{}_outside_{}_{}[{}] : -99".format(
                                WP, boosted_jets, var, ijet, WP, boosted_jets, var, ijet
                            )
                        )
...
def main(input_file, output_dir, year, is_data, is_sgn, weighting, sys):
    ...
    # input_file = "/nfs/dust/cms/user/gmilella/ttX_ntuplizer/bkg_2016_hotvr/merged/ttH_HToNonbb_MC2016_ntuplizer_merged.root"
