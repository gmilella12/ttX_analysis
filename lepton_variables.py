...
EVENT_SELECTION = "after_2OS_2ak4_2b_outside_{}".format(BOOSTED_JETS)
...
class Processor:
    def __init__(self, input_file, output_dir, year, is_data, is_sgn, weighting, sys):
        ...
        self.output_file = creation_output_file(input_file, output_dir, "lepton_variables", year, EVENT_SELECTION, self.sys) 
        ...
        for lepton_selection in self.lepton_selection:
            ...
            for var in VARIABLES_BINNING.keys():
                ...
                if var == 'invariant_mass_leading_subleading':
                    ...
                    if lepton_selection == 'emu':
                        histo_var = 'dilepton_{}'.format(var)
                    elif lepton_selection == 'ee':
                        histo_var = '{}_{}_Electrons_{}'.format(LEPTON_ID_ELE, ELECTRON_ID_TYPE, var)
                    elif lepton_selection == 'mumu':
                        histo_var = 'tightRelIso_{}ID_Muons_{}'.format(LEPTON_ID_MUON, var)
                    ...
                    for ijet, jet in enumerate(['leading', 'subleading']):
                        histo_title = "{}_{}_{}_{}_{}".format(self.process_name, jet, var, EVENT_SELECTION, lepton_selection)
                        if lepton_selection == 'ee':
                            histo_var = '{}_{}_Electrons_{}_{}'.format(LEPTON_ID_ELE, ELECTRON_ID_TYPE, var, jet)
                        elif lepton_selection == 'mumu':
                            histo_var = 'tightRelIso_{}ID_Muons_{}_{}'.format(LEPTON_ID_MUON, var, jet)
                        else: continue
                    ...
                    if lepton_selection == 'emu':
                        for lepton, lepton_label in zip(['electron', 'muon'],
                                                         ['{}_{}_Electrons_{}'.format(LEPTON_ID_ELE, ELECTRON_ID_TYPE, var),
                                                           'tightRelIso_{}ID_Muons_{}'.format(LEPTON_ID_MUON, var)]):

                            histo_title = "{}_{}_{}_{}_{}".format(self.process_name, lepton, var, EVENT_SELECTION, lepton_selection)
                            histo_var = '{}'.format(lepton_label)
    ...
    def _adding_new_columns(self, root_df):
        ...
        for var in VARIABLES_BINNING.keys():
            if var == 'invariant_mass_leading_subleading':
                ...
                for ijet, jet in enumerate(['leading', 'subleading']): 
                    root_df = root_df.Define(
                        "{}_{}_Electrons_{}_{}".format(LEPTON_ID_ELE, ELECTRON_ID_TYPE, var, jet),
                        "{}_{}_Electrons_{}.size() > {} ? {}_{}_Electrons_{}[{}] : -99".format(
                            LEPTON_ID_ELE, ELECTRON_ID_TYPE, var, ijet, LEPTON_ID_ELE, ELECTRON_ID_TYPE, var, ijet
                        )
                    ) # always checking the size of the array; if empty, segmentation violation error arises
                    root_df = root_df.Define(
                        "tightRelIso_{}ID_Muons_{}_{}".format(LEPTON_ID_MUON, var, jet),
                        "tightRelIso_{}ID_Muons_{}.size() > {} ? tightRelIso_{}ID_Muons_{}[{}] : -99".format(
                            LEPTON_ID_MUON, var, ijet, LEPTON_ID_MUON, var, ijet
                        )
                    ) # always checking the size of the array; if empty, segmentation violation error arises
        # ---
...
def main(input_file, output_dir, year, is_data, is_sgn, weighting, sys):
    ...
    # input_file = "/nfs/dust/cms/user/gmilella/ttX_ntuplizer/bkg_2016preVFP_hotvr/merged/dy_ht_inf_MC2016preVFP_ntuplizer_merged.root"
